...
public class UserAgent extends CallListenerAdapter {
	...
	public static final int UA_STATE_IDLE = 0;     			//²¦ºÅ×´Ì¬
	public static final int UA_STATE_INCOMING_CALL = 1;		//À´µç×´Ì¬
	public static final int UA_STATE_OUTGOING_CALL = 2;		//¹Ò¶Ï×´Ì¬
	public static final int UA_STATE_INCALL = 3;			//Í¨»°×´Ì¬
	public static final int UA_STATE_HOLD = 4;				//´ý»ú×´Ì¬

	int call_state = UA_STATE_IDLE;  	//Ä¬ÈÏ×´Ì¬£¬´ý»ú
...
//	private void addMediaDescriptor(String media, int port, Codecs.Map c,int rate) {
	...
	public boolean call(String target_url, boolean send_anonymous) {
		...
		if (target_url.indexOf("@") < 0) {
			if (user_profile.realm.equals(SipConfig.SERVER))
