			...
			for (Enumeration<NetworkInterface> en = NetworkInterface.getNetworkInterfaces(); en.hasMoreElements();) {
				...
				for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements();) {
					...
					if (!inetAddress.isLoopbackAddress()) { 
						if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(SipConfig.PREF_STUN, SipConfig.DEFAULT_STUN)) {
							localIpAddress = inetAddress.getHostAddress().toString();
						} else {
							try {
								String StunServer = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_STUN_SERVER, SipConfig.DEFAULT_STUN_SERVER);
								int StunServerPort = Integer.valueOf(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_STUN_SERVER_PORT, SipConfig.DEFAULT_STUN_SERVER_PORT));
