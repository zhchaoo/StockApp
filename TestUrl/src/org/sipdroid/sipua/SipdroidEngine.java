...
public class SipdroidEngine implements RegisterAgentListener {
	...
	UserAgentProfile getUserAgentProfile(String suffix) {
		...
		user_profile.username = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_USERNAME+suffix, SipConfig.USERNAME); // modified
		user_profile.passwd = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_PASSWORD+suffix, SipConfig.PASSWORD);
		if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_DOMAIN+suffix, SipConfig.DEFAULT_DOMAIN).length() == 0) {
			user_profile.realm = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_SERVER+suffix, SipConfig.SERVER);
		} else {
			user_profile.realm = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_DOMAIN+suffix, SipConfig.DEFAULT_DOMAIN);
		}
		user_profile.realm_orig = user_profile.realm;
		if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_FROMUSER+suffix, SipConfig.DEFAULT_FROMUSER).length() == 0) {
			user_profile.from_url = user_profile.username;
		} else {
			user_profile.from_url = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_FROMUSER+suffix, SipConfig.DEFAULT_FROMUSER);
		}
		
		// MMTel configuration (added by mandrajg)
		user_profile.qvalue = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_MMTEL_QVALUE, SipConfig.DEFAULT_MMTEL_QVALUE);
		user_profile.mmtel = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(SipConfig.PREF_MMTEL, SipConfig.DEFAULT_MMTEL);

		user_profile.pub = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(SipConfig.PREF_EDGE+suffix, SipConfig.DEFAULT_EDGE) ||
			PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(SipConfig.PREF_3G+suffix, SipConfig.DEFAULT_3G);
		return user_profile;
	}
	...
	public boolean StartEngine() {
			...
			if (wl == null) {
				if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).contains(SipConfig.PREF_KEEPON)) {
					Editor edit = PreferenceManager.getDefaultSharedPreferences(getUIContext()).edit();
	
					edit.putBoolean(SipConfig.PREF_KEEPON, Build.MODEL.equals("Nexus One") ||
			...
			pref = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getInt(SipConfig.PREF_ACCOUNT, SipConfig.DEFAULT_ACCOUNT);
			...
			for (UserAgentProfile user_profile : user_profiles) {
				if (wl[i] == null) {
					...
					if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(SipConfig.PREF_KEEPON, SipConfig.DEFAULT_KEEPON))
		...
		//			SipStack.log_path = "/data/data/org.sipdroid.sipua";
					...
					SipStack.default_transport_protocols[0] = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_PROTOCOL+(i!=0?i:""),
							user_profile.realm.equals(SipConfig.SERVER)?"tcp":"udp");
					
					String version = "Sipdroid/" + Voip.getVersion() + "/" + Build.MODEL;
	...
	void setOutboundProxy(SipProvider sip_provider,int i) {
		try {
			if (sip_provider != null) sip_provider.setOutboundProxy(new SocketAddress(
					IpAddress.getByName(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_DNS+i, SipConfig.DEFAULT_DNS)),
					Integer.valueOf(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_PORT+(i!=0?i:""), SipConfig.DEFAULT_PORT))));
		} catch (Exception e) {
		}
	}
	...
	public void updateDNS() {
		...
		for (SipProvider sip_provider : sip_providers) {
			try {
				edit.putString(SipConfig.PREF_DNS+i, IpAddress.getByName(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_SERVER+(i!=0?i:""), "")).toString());
	...
	public boolean call(String target_url,boolean force) {
		...
		if (!found || (ua = uas[p]) == null) {
			if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(SipConfig.PREF_CALLBACK, SipConfig.DEFAULT_CALLBACK) &&
					PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(SipConfig.PREF_POSURL, SipConfig.DEFAULT_POSURL).length() > 0) {
		...
		for (KeepAliveSip ka : kas) {
			if (ka != null && Receiver.on_wlan && isRegistered(i))
				try {
					...
					if (!org.sipdroid.Sipdroid.release) e.printStackTrace();
				}
			i++;
		}
	}
