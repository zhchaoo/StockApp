...
import com.chaos.testurl.TestUrl;
import com.chaos.testurl.R;
import jxt.app.voip.SipConfig;
import jxt.app.voip.Voip;

import org.sipdroid.Sipdroid;
	...
	public class Receiver extends BroadcastReceiver {
			...
			if (call_state != state) {
				...
				case UserAgent.UA_STATE_INCOMING_CALL:
					...
					if ((pstn_state == null || pstn_state.equals("IDLE")) &&
							PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_AUTO_ON, SipConfig.DEFAULT_AUTO_ON) &&
						...
						if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {				 
							String sUriSipRingtone = PreferenceManager.getDefaultSharedPreferences(mContext).getString(SipConfig.PREF_SIPRINGTONE,
				...
				case UserAgent.UA_STATE_IDLE:
					...
					if (wl != null && wl.isHeld())
						wl.release();
			        mContext.startActivity(createIntent(TestUrl.class));
		...
		public static void onText(int type,String text,int mInCallResId,long base) {
			...
			if (type >= REGISTER_NOTIFICATION && mInCallResId == R.drawable.sym_presence_available &&
					!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(SipConfig.PREF_REGISTRATION, SipConfig.DEFAULT_REGISTRATION))
				text = null;
	        ...
	        if (text != null) {
				...
				if (type == MISSED_CALL_NOTIFICATION) {
			        	...
			        	if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(SipConfig.PREF_NOTIFY, SipConfig.DEFAULT_NOTIFY)) {
...
//				                createIntent(AutoAnswer.class), 0);
					...
					if (base != 0) {
						...
						if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_POS, SipConfig.DEFAULT_POS))
		...
		static void updateAutoAnswer() {
			if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_AUTO_ONDEMAND, SipConfig.DEFAULT_AUTO_ONDEMAND) &&
				Voip.on(mContext)) {
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_AUTO_DEMAND, SipConfig.DEFAULT_AUTO_DEMAND))
					updateAutoAnswer(1);
				else
					updateAutoAnswer(0);
			} else
		...
		public static void pos(boolean enable) {
			...
			pos_gps(false);
			if (enable) {
				if (call_state == UserAgent.UA_STATE_IDLE && Voip.on(mContext) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_POS, SipConfig.DEFAULT_POS) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getString(SipConfig.PREF_POSURL, SipConfig.DEFAULT_POSURL).length() > 0) {
		...
		static void enable_wifi(boolean enable) {
			if (!PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_OWNWIFI, SipConfig.DEFAULT_OWNWIFI))
				return;
			if (enable && !PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_WIFI_DISABLED, SipConfig.DEFAULT_WIFI_DISABLED))
        		return;
    		...
    		edit.putBoolean(SipConfig.PREF_WIFI_DISABLED,!enable);
    		edit.commit();
    		if (enable) {
		...
		public static void url(final String opt) {
	        (new Thread() {
				public void run() {
					try {
				        URL url = new URL(PreferenceManager.getDefaultSharedPreferences(mContext).getString(SipConfig.PREF_POSURL, SipConfig.DEFAULT_POSURL)+
				        		"?"+opt);
				        BufferedReader in;
		...
		static boolean on_vpn() {
			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_ON_VPN, SipConfig.DEFAULT_ON_VPN);
		}
		
		static void on_vpn(boolean enable) {
    		Editor edit = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).edit();
    		
    		edit.putBoolean(SipConfig.PREF_ON_VPN,enable);
    		edit.commit();
		}
		...
		public static boolean isFast(int i) {
        	...
        	if (PreferenceManager.getDefaultSharedPreferences(mContext).getString(SipConfig.PREF_USERNAME+(i!=0?i:""),"").equals("") ||
        			PreferenceManager.getDefaultSharedPreferences(mContext).getString(SipConfig.PREF_SERVER+(i!=0?i:""),"").equals(""))
        		return false;
        	if (wi != null) {
	        	...
	        	if (wi.getIpAddress() != 0 && (WifiInfo.getDetailedStateOf(wi.getSupplicantState()) == DetailedState.OBTAINING_IPADDR
	        		...
	        		on_wlan = true;
	        		if (!on_vpn())
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_WLAN+(i!=0?i:""), SipConfig.DEFAULT_WLAN);
	        		else
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_VPN+(i!=0?i:""), SipConfig.DEFAULT_VPN);  
	        	}
        	}
        	on_wlan = false;
			return isFastGSM(i);
		...
		static boolean isFastGSM(int i) {
        	...
        	if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
        		return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_3G+(i!=0?i:""), SipConfig.DEFAULT_3G);
        	if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
       			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_EDGE+(i!=0?i:""), SipConfig.DEFAULT_EDGE);
        	return false;
		}
		...
		public void onReceive(Context context, Intent intent) {
	        ...
	        if (intentAction.equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
	        	if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(SipConfig.PREF_SELECTWIFI, SipConfig.DEFAULT_SELECTWIFI)) {
		        	WifiManager wm = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
		        	WifiInfo wi = wm.getConnectionInfo();
	    ...
	    private static OnCallRefusedListener mRefusedListener;
	    
	    public static void setOnCallRefusedListener(OnCallRefusedListener listener) {
	    	mRefusedListener = listener;
	    }
	    
	    public static void onCallRefused(String reason) {
	    	if(mRefusedListener != null) {
	    		mRefusedListener.onCallRefused(reason);
	    	}
	    }
	    
	    public interface OnCallRefusedListener {
