...
public class Caller extends BroadcastReceiver {
		...
		public void onReceive(final Context context, Intent intent) {
	        ...
	        if (intentAction.equals(Intent.ACTION_NEW_OUTGOING_CALL) && number != null)
	        {
				...
				if (sip_type && !force) {
	    			String sExPat = PreferenceManager.getDefaultSharedPreferences(context).getString(SipConfig.PREF_EXCLUDEPAT, SipConfig.DEFAULT_EXCLUDEPAT); 
    			...
    			else 
    			{
	        		if (number != null && !intent.getBooleanExtra("android.phone.extra.ALREADY_CALLED",false)) {
	        		    	...
	        		    	if (sp.contains("prefix")) {
	        		    	    String prefix = sp.getString(SipConfig.PREF_PREFIX, SipConfig.DEFAULT_PREFIX);
	        		    	    Editor editor = sp.edit();
	        		    	    if (!prefix.trim().equals("")) {
	        		    		editor.putString(SipConfig.PREF_SEARCH, "(.*)," + prefix + "\\1");
	        		    	    }
	        		    	    editor.remove(SipConfig.PREF_PREFIX);
	        		    	    editor.commit();
	        		    	}
	        		    	
	        		    	// Search & replace.
	    				String search = sp.getString(SipConfig.PREF_SEARCH, SipConfig.DEFAULT_SEARCH);
	    				String callthru_number = searchReplaceNumber(search, number);
	    				String callthru_prefix;
	    				
						if (!ask && !force && PreferenceManager.getDefaultSharedPreferences(context).getBoolean(SipConfig.PREF_PAR, SipConfig.DEFAULT_PAR)) 
	    				{
...
//	        				}        					
	    				...
	    				if (!ask && Receiver.engine(context).call(number,force))
	    					setResultData(null);
	    				else if (!ask && PreferenceManager.getDefaultSharedPreferences(context).getBoolean(SipConfig.PREF_CALLTHRU, SipConfig.DEFAULT_CALLTHRU) &&
	    						(callthru_prefix = PreferenceManager.getDefaultSharedPreferences(context).getString(SipConfig.PREF_CALLTHRU2, SipConfig.DEFAULT_CALLTHRU2)).length() > 0) {
